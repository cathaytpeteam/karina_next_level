...
async def fill_protect(r, n="401", zone="B", gate="5", dep="1945", go=None, target="gProtectedFlight"):
...
async def priority_suite():
    ...
    async with async_playwright() as p:
        ...
        ck("[priority] S1 progress wording uses Join Passenger", (await r.pg.locator("#step").inner_text()).strip() == "漏查 - 2/3 - Join Passenger", (await r.pg.locator("#step").inner_text()).strip())
        ...
        ck("[priority] S2 progress wording uses Join Passenger", "Join Passenger" in (await r.pg.locator("#step").inner_text()))
...
async def case_direct(r):
    # R1.2: Call Directly is its own entry on the Scenario page.
...
async def rule(r, name):
    if name == "s1_mode_switch_clears_inputs":
        ...
        ck("[rule] ... and keeps Protect to", [await r.val(x) for x in ("gNewN", "gGate", "gDepTime")] == ["401", "5", "19:45"] and s_["pressed"] == ["gProtectedFlight", "gpWait"], str(s_["pressed"]))
...
async def _sw_mode(p, tag, noroutes, port, previous, tmp):
    ...
    ck(T + "1 first visit: one cache with all 14 assets", len(cs) == 1 and list(cs.values()) == [14], str(cs))
    ...
    ck(T + "3 offline: cache intact", sum((await pg.evaluate(_CACHE_JS)).values()) == 14); await pg.close()
    srv.start()
